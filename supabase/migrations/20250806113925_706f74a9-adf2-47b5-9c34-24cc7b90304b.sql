-- Fix security warnings: Set search_path for database functions with proper signatures
-- This prevents function search path mutable security issues

-- Fix search_path for functions with unique signatures
...
ALTER FUNCTION public.process_refund(uuid, integer, text, text) SET search_path = public;
ALTER FUNCTION public.reset_daily_credits_if_needed(uuid) SET search_path = public;
ALTER FUNCTION public.use_credits(uuid, numeric, text) SET search_path = public;
ALTER FUNCTION public.add_credits_to_user(uuid, numeric, text, text) SET search_path = public;
ALTER FUNCTION public.use_tokens(uuid, bigint, text) SET search_path = public;
ALTER FUNCTION public.add_tokens_to_user(uuid, bigint, text, text, text) SET search_path = public;
ALTER FUNCTION public.reset_daily_tokens_if_needed(uuid) SET search_path = public;
